...
{
	if (maxAge == 0)
		return false; // maxAge of 0 means nothing is in the cache and we're not caching anything
	...
	{ // if it's not valid, then remove it
		cout << oslock << "     [Cache entry with hash of " << requestHash << " has expired... removing...]" << endl
			 << osunlock;
	...
	try
	{
		...
		cout << oslock << "     [Using cached copy of previous request for " << request.getURL() << ".]" << endl
			 << osunlock;
		return true;
	}
...
string HTTPCache::getRequestHashCacheEntryName(const string &requestHash) const
{
	...
	if (dir == NULL)
		return ""; // just assume it doesn't exist
	...
	while (true)
	{
		...
		if (cachedEntryName != "." && cachedEntryName != "..")
			break;
	}
...
string HTTPCache::getCurrentTime() const
{ // not influences by maxAge at all
	struct timeval tv;
...
bool HTTPCache::cachedEntryIsValid(const string &cachedFileName) const
{
	...
	if (maxAge > 0)
		expirationTime = min<long>(createTime + maxAge, expirationTime);
	cout << oslock << "     [Cache entry created at " << createTime << ", expires at " << expirationTime << ".]" << endl
		 << osunlock;
	struct timeval tv;
	gettimeofday(&tv, NULL); // no error possible when just getting the time
