...
static string rtrim(const string &s)
{
  ...
  {
    if (!isspace(static_cast<unsigned char>(s[end])))
      break;
  }
  size_t len = end + 1;
...
static const string trim(const string &s)
{
  ...
  {
    if (!isspace(static_cast<unsigned char>(s[start])))
      break;
  }
  for (end = s.length() - 1; end >= 0; end--)
  {
...
static const bool endsWith(std::string const &str, std::string const &suffix)
{
